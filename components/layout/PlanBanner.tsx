...
import { PLAN_LABELS, exceedsSubscriptionLimit } from "@/lib/entitlements/catalog";
...
export function PlanBanner({ entitlements, subscriptionCount }: { entitlements: Entitlements; subscriptionCount: number }) {
  ...
  if (!exceedsSubscriptionLimit(entitlements, subscriptionCount) || max === null) return null;
  const next = entitlements.plan === "assessment" ? PLAN_LABELS.starter : PLAN_LABELS.pro;
  return (
    <div className="no-print rounded-xl border border-amber-400/20 bg-amber-400/10 px-4 py-3 text-xs text-amber-100">
